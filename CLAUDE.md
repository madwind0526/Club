...
- **`server/backup.ts`** — Node-only module backing "DB 백업"/"DB 복원": `createDatabaseBackup()` zips `data/runtime/*.json` + the whole resolved data-root folder + `<projectRoot>/Report` into `<projectRoot>/backup/YYYY-MM-DD-DB backup.zip` (via `jszip` - not `adm-zip`, which some corporate environments block); `restoreDatabaseBackup()` deletes those same three (current, re-read from `app-settings.json` *before* deleting it) and re-extracts the chosen zip's `data/runtime/`, `data-root/`, `report/` prefixes back into them; `listDatabaseBackups()` lists what's in `backup/`. `export:monthlyExcel`/`/api/export-monthly-excel` call `createDatabaseBackup()` fire-and-forget after a successful export, and so does `ActivityReportView.tsx`'s `handleSave` when `isAdmin` (not for a non-admin's self-attendance-only save through the same button - deliberately excluded, since it's too frequent for a full data-root re-zip each time).
