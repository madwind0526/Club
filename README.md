...
- **활동 계획서**: 수동 첨부 또는 `Plan` 폴더에서 파일명 규칙으로 자동 첨부, 썸네일(이미지/문서 아이콘)로 표시 — 클릭 시 이미지·PDF는 팝업 미리보기, 문서 파일(doc/xls/ppt)은 OS 기본 프로그램으로 열림
...
### 회원 관리 (admin 전용)
- 회원 추가/수정/삭제
- **불러오기**: 파일 선택 창에서 직접 선택 / **자동불러오기**: `assets/members.json` 또는 `assets/members.txt`를 자동으로 읽음 / **내보내기**
...
- 참석자별 1차·2차·... 출석 현황 + 참석 횟수에 따른 후원금액(1회 5만원 / 2회 이상 10만원) 자동 계산
- 사진 / 영수증 사진·내역 / 경비 사진·내역을 주차별로 모아서 표시
- **Excel로 내보내기** — `.xlsx` 한 파일에 아래 시트를 생성 (모든 표에 테두리 적용)
  - **Summary**: 이달의 활동 수·참석 인원 2x2 표 + 참석자 표를 좌측에, 사진/영수증(사진+표)/경비(사진+표)를 오른쪽으로 나란히 배치
  - **N주차** (활동이 있는 주만): 제목, 활동 계획서, 참석자 표, 사진, 영수증(사진+표), 경비(사진+표)를 그 주 리포트 그대로 정리
...
### Settings
- 클럽 이름/로고/소개, 데이터 루트 폴더, 회원 불러오기 형식/방식, 테마(라이트/다크, 버튼 클릭 시 즉시 적용)
...
<데이터 루트 폴더>/
  ...
  Plan/YYYY-MM-WeekN.<확장자>    예: Plan/2026-07-Week3.jpg
```

- Photos/Receipts/Expenses는 해당 폴더 안의 모든 이미지 파일(jpg/jpeg/png/gif/webp/bmp)을 불러옵니다.
- Plan은 파일명이 `YYYY-MM-WeekN`으로 **시작하는** 파일 하나를 찾습니다(예: `2026-07-Week3.jpg`). 지원 확장자: jpg, jpeg, gif, png, doc, docx, xls, xlsx, ppt, pptx, pdf.

### 회원 자동불러오기 (`assets/`)

Settings에서 형식을 TXT로 두면 `assets/members.txt`(탭으로 구분, 엑셀의 "텍스트(탭으로 구분)" 저장 형식과 동일), JSON으로 두면 `assets/members.json`을 회원 관리 화면의 "자동불러오기" 버튼이 읽습니다. 열 순서: 이름, Knox ID, 부서, 연락처, 가입 날짜, 회원 등급, 역할, 비고.
